...
Начиная с 0.4.0rc1 порядок `altorder=1` сохраняется встроенной Lua-функцией совместимости. Это информационная запись; `--strict` её не отклоняет. Порядок первого запуска: [ИНСТРУКЦИЯ.txt](../ИНСТРУКЦИЯ.txt).
